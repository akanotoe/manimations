...
class FourierSceneAbstract(Scene):
    ...
    def get_points(self, path):
        '''This gets points along the path at a uniform spacing. To
           get points more densely spaced when curvature is higher,
           see the `get_points` method in ``FourierOfSVG``.'''
        n = self.path_n_samples
        dt = 1 / n
